...
class _SettingsViewState extends State<SettingsView> {
  ...
  Future<void> _loadProfile() async {
    // Load display name from local storage, default to "Deviinee"
    try {
      final box = await Hive.openBox('profileBox');
      final saved = box.get('displayName') as String?;
      userName = (saved == null || saved.trim().isEmpty) ? "Deviinee" : saved.trim();
    } catch (_) {
      userName = "Deviinee";
    }
    // Load email from Supabase auth if available
    try {
  ...
  Future<void> _saveDisplayName(String name) async {
    try {
      final box = await Hive.openBox('profileBox');
      await box.put('displayName', name.trim());
    } catch (_) {}
  }
        ...
        child: SafeArea(
          child: Column(children: [
            ...
            TextButton(
              ...
              child: Text(
                "Edit name",
                style: TextStyle(color: TColor.white, fontSize: 12, fontWeight: FontWeight.w600),
              ),
            ),
            Padding(
                              ...
                              if (pendingAfter > 0) {
                                showDialog(
                                  ...
                                  builder: (ctx) => AlertDialog(
                                    ...
                                    content: Text(
                                      'You still have $pendingAfter change(s) waiting to sync. Please connect to the internet and try again.',
                                    ),
                                    actions: [
                                      TextButton(
